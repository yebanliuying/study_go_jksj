...
func List(ctx *gin.Context) {
	r, err := global.GoodsSrvCln.GetAllCategorysList(context.Background(), &empty.Empty{})
	if err != nil {
...
func Detail(ctx *gin.Context) {
	...
	if r, err := global.GoodsSrvCln.GetSubCategory(context.Background(), &proto.CategoryListRequest{
		Id: int32(i),
	}); err != nil {
		api.HandleGrpcErrorToHttp(err, ctx)
		return
...
func New(ctx *gin.Context) {
	...
	rsp, err := global.GoodsSrvCln.CreateCategory(context.Background(), &proto.CategoryInfoRequest{
...
func Delete(ctx *gin.Context) {
	...
	_, err = global.GoodsSrvCln.DeleteCategory(context.Background(), &proto.DeleteCategoryRequest{Id: int32(i)})
	if err != nil {
		api.HandleGrpcErrorToHttp(err, ctx)
		return
	}
...
func Update(ctx *gin.Context) {
	...
	_, err = global.GoodsSrvCln.UpdateCategory(context.Background(), request)
	if err != nil {
		api.HandleGrpcErrorToHttp(err, ctx)
		return
	}
